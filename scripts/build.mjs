...
// venue-report label + description maps emit in REPORT_REASONS (canonical) order so both outputs are
// stable — mirrors the ATTRIBUTE_LABELS label-map lever above (flat string→string, same shape).
const tsReportLabelEntries = reportReasons.map((k) => `${k}: ${JSON.stringify(reportReasonLabels[k])}`).join(", ");
const ktReportLabelEntries = reportReasons.map((k) => `${JSON.stringify(k)} to ${JSON.stringify(reportReasonLabels[k])}`).join(", ");
const tsReportDescEntries = reportReasons.map((k) => `${k}: ${JSON.stringify(reportReasonDescriptions[k])}`).join(", ");
const ktReportDescEntries = reportReasons.map((k) => `${JSON.stringify(k)} to ${JSON.stringify(reportReasonDescriptions[k])}`).join(", ");

// Each report-copy map must carry exactly one entry per REPORT_REASONS key — no missing, no stray.
// Kotlin's Map has no compile-time key-completeness check, so this is the single cross-language guard:
// drift aborts the build (RED) rather than emitting a divergent contract (mirrors the ATTRIBUTE_LABELS
// guard above). The keys are the venue_reports rule set; the values are web's hand-coded report copy.
for (const [name, map] of [
  ["reportReasonLabels", reportReasonLabels],
  ["reportReasonDescriptions", reportReasonDescriptions],
]) {
  const missing = reportReasons.filter((k) => !(k in map));
  const stray = Object.keys(map).filter((k) => !reportReasons.includes(k));
  if (missing.length > 0 || stray.length > 0) {
    throw new Error(
      `community.json: ${name} keys must match reportReasons exactly ` +
 ...
 */
export const REPORT_REASON_LABELS: Record<ReportReason, string> = { ${tsReportLabelEntries} };
 ...
 */
export const REPORT_REASON_DESCRIPTIONS: Record<ReportReason, string> = { ${tsReportDescEntries} };
...
object Community {
     ...
     */
    val REPORT_REASON_LABELS: Map<String, String> = mapOf(${ktReportLabelEntries})
     ...
     */
    val REPORT_REASON_DESCRIPTIONS: Map<String, String> = mapOf(${ktReportDescEntries})
